  ...
  return (
    <div className="flex min-h-screen bg-cream">
      ...
      <div className={`fixed inset-x-0 top-0 z-40 flex h-14 items-center justify-between border-b px-4 md:hidden ${dark ? 'border-white/10 bg-navy text-paper' : 'border-ink/8 bg-paper text-ink'}`}>
        ...
        <div className="flex items-center gap-3"><ThemeToggle compact /><Link to="/perfil" className="text-sm font-bold text-terracotta">Mi perfil</Link><button onClick={() => { setToken(null); navigate('/entrar') }} className="text-sm font-bold text-ink-soft">Salir</button></div>
      </div>
