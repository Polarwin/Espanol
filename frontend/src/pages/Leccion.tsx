...
export function Leccion() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-5 sm:px-8 sm:py-8">
      ...
      {lesson.title === 'Vitamina A2 · U1: Vamos a conocernos' && <Link to="/muestra/a2/1" className="mt-4 block rounded-2xl bg-river p-4 font-bold text-paper">Probar la nueva Unidad 1: vocabulario del libro, explicaciones en español y revisión de escritura →</Link>}
      ...
      <div className="mt-6 grid gap-3 sm:ml-auto sm:flex sm:w-fit"><Link to={`/leccion/${lesson.id}/repetir-video`} className="rounded-2xl border-2 border-river px-6 py-3 text-center font-bold text-river">Escuchar y repetir el vídeo</Link><Link to={`/practica/conversacion/${lesson.id}`} className="rounded-2xl border-2 border-terracotta px-6 py-3 text-center font-bold text-terracotta">Conversar sobre esta unidad</Link><Link to={`/leccion/${lesson.id}/prueba`} className="rounded-2xl bg-terracotta px-6 py-3.5 text-center font-bold text-paper shadow-card">{user ? `${preferredName(user)}, empezar la práctica` : 'Empezar la práctica'}</Link></div>
      <Link to={`/practica/conversacion/${lesson.id}`} className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] right-4 z-40 flex items-center gap-2 rounded-full bg-terracotta px-4 py-3 font-extrabold text-paper shadow-card md:hidden" aria-label="Conversar sobre esta unidad"><IconMic size={20} />Conversar</Link>
    </div>
  )
}
