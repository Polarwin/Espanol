...
export function Conversacion() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setError('No se pudo preparar esta conversación.'))
  }, [lessonId, reset])
  ...
  return <div className="mx-auto max-w-3xl px-4 py-5 sm:px-8 sm:py-8"><div className="rounded-3xl bg-[linear-gradient(135deg,#17324d,#24566f)] p-5 text-paper shadow-card sm:p-7"><p className="text-xs font-bold uppercase tracking-[.18em] text-sun">Conversación {setup?.cefr_level ?? ''}</p><h1 className="mt-2 font-display text-3xl font-bold">{setup?.title ?? 'Preparando conversación…'}</h1><p className="mt-2 text-paper/80">{setup?.goal ?? 'Responde con tus propias palabras.'}</p>{setup && <div className="mt-3 flex flex-wrap gap-2">{setup.vocabulary.map((word) => <span key={word} className="rounded-full bg-paper/12 px-3 py-1 text-xs font-bold">{word}</span>)}</div>}</div>
