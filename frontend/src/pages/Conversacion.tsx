...
const greeting = (name: string) => `¡Hola, ${name}! Soy Ana, tu vecina. ¿Cómo estás hoy?`
...
export function Conversacion() {
  ...
  const [learnerName, setLearnerName] = useState('amigo')

  useEffect(() => {
    api.getMe().then((user) => {
      const name = preferredName(user)
      setLearnerName(name)
      setMessages((old) => old.length === 1 ? [{ role: 'tutor', text: greeting(name) }] : old)
    }).catch(() => {})
  }, [])
  ...
  const restart = () => { setTurn(0); setResult(null); setMessages([{ role: 'tutor', text: greeting(learnerName) }]); reset() }
