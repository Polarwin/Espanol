...
export function Lecciones() {
  ...
  useEffect(() => { api.getLessons().then(setLessons).catch(() => setError('No se pudieron cargar las lecciones.')) }, [])

  return (
    <div className="mx-auto max-w-5xl px-4 py-5 sm:px-8 sm:py-8">
      <div className="flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-full bg-river-soft text-river"><IconBook size={24} /></span><div><h1 className="font-display text-2xl font-bold sm:text-3xl">Lecciones</h1><p className="font-semibold text-ink-soft">Vídeo, diálogo, frases y práctica en cada lección.</p></div></div>
